...
public struct DocSchemaMacro: MemberMacro {
    ...
    private static func extractMacroArguments(from node: AttributeSyntax) throws -> (name: String?, description: String?, schema: String?) {
    ...
    public static func generateDescriptionProperty(description: String, in structDecl: StructDeclSyntax) throws -> DeclSyntax? {
    ...
    private static func generateInputSchemaProperty(from schemaInfo: SchemaInfo, in structDecl: StructDeclSyntax, context: some MacroExpansionContext) -> DeclSyntax {
        let accessModifier = getAccessModifier(from: structDecl)
    ...
    private static func generateSchemaInstanceInputSchemaProperty(from schemaExpression: String, in structDecl: StructDeclSyntax, context: some MacroExpansionContext) -> DeclSyntax {
        let accessModifier = getAccessModifier(from: structDecl)
    ...
    private static func generateSchemaJson(from schemaInfo: SchemaInfo, in structDecl: StructDeclSyntax, context: some MacroExpansionContext) -> String {
    ...
    public static func generateSingleParameterSchema(_ param: ParameterInfo, in structDecl: StructDeclSyntax, context: some MacroExpansionContext) -> String {
    ...
    private static func generateObjectSchema(from schemaInfo: SchemaInfo) -> String {
        ...
        for param in schemaInfo.parameters {
            ...
            if let description = param.description {
                propertySchema = "\"\(param.name)\": { \"type\": \"\(typeSchema)\", \"description\": \"\(description)\" }"
            } else {
    ...
    private static func findHandlerFunctionWithDocComment(in structDecl: StructDeclSyntax) throws -> (FunctionDeclSyntax, String) {
        for member in structDecl.memberBlock.members {
